...
describe('mockHermesBridge', () => {
  ...
  test('creates directly assigned pet tasks for the active profile', () => {
    ...
    const taskId = bridge.createTask({
      brief: 'Prepare a test quest report.',
      assigneeId: 'builder',
      type: 'pet',
    });
    const snapshot = bridge.getSnapshot();
  ...
  test('keeps advanced quest intake fields with the task', () => {
    const bridge = createMockHermesBridge();
    const taskId = bridge.createTask({
      brief: 'Prepare a scoped implementation pass.',
      goals: 'Ship the narrow vertical loop.',
      nonGoals: 'Do not add Tavern or XP.',
    ...
    expect(task?.nonGoals).toBe('Do not add Tavern or XP.');
  ...
  test('completes mock execution with a reviewable report card', async () => {
    const bridge = createMockHermesBridge();
    const taskId = bridge.createTask({
      brief: 'Prepare a test quest report.',
      assigneeId: 'builder',
  ...
  test('persistent bridge mode stores and restores a snapshot envelope', () => {
    ...
    const stored = globalThis.localStorage.getItem('hermes-guild.mock-bridge.snapshot');
  ...
  test('persistent bridge mode sanitizes invalid restored profile references', () => {
    ...
    globalThis.localStorage.setItem(
      'hermes-guild.mock-bridge.snapshot',
      JSON.stringify({
        origin: 'external',
  ...
  test('persistent bridge mode receives storage event fallback updates', () => {
    ...
    const stored = globalThis.localStorage.getItem('hermes-guild.mock-bridge.snapshot');
    expect(stored).toBeString();
    storageListeners.forEach((listener) => listener({ key: 'hermes-guild.mock-bridge.snapshot', newValue: stored }));
  ...
  test('persistent bridge mode receives BroadcastChannel updates', () => {
    ...
    const sendingBridge = createMockHermesBridge({ persist: true });
    const taskId = sendingBridge.createTask({
      brief: 'Broadcast this mock quest.',
      assigneeId: 'builder',
      type: 'pet',
    });
  ...
  test('persistent bridge storage fallback stops local timers when a remote update blocks the task', async () => {
    ...
    const createdEnvelope = globalThis.localStorage.getItem('hermes-guild.mock-bridge.snapshot');
    expect(createdEnvelope).toBeString();
    storageListeners.forEach((listener) => listener({ key: 'hermes-guild.mock-bridge.snapshot', newValue: createdEnvelope }));

    remoteControlBridge.simulateBlocked(taskId);
    const blockedEnvelope = globalThis.localStorage.getItem('hermes-guild.mock-bridge.snapshot');
    expect(blockedEnvelope).toBeString();
    storageListeners.forEach((listener) => listener({ key: 'hermes-guild.mock-bridge.snapshot', newValue: blockedEnvelope }));
    await wait(4700);
